...
from text.components import ContextualQA, Document
...
class DocumentsCollectionHandler(ChromaHandler):
    ...
    Methods
    -------
    `add_entry(doc: Document, stricted: bool = True)` -> None
        Adds a document to the Chroma collection, processing each page and
        chunk, with optional strict checking for embeddings.
    """
...
class ContextualQuestionHandler(ChromaHandler):
    ...
    questions generated by an LM.

    Attributes
    ----------
    QA_AMOUNT : int
        The number of question-answer pairs to handle per entry.

    Methods
    -------
    `add_entry(cqa: ContextualQA)` -> None
        Adds a ContextualQA entry to the "contextual_questions"
        collection.
    """
...
class ContextualResponseHandler(ChromaHandler):
    """Handler for managing in a Chroma database the responses generated
    relative to contextualized questions.

    Attributes
    ----------
    QA_AMOUNT : int
        The number of question-answer pairs to handle per context.

    Methods
    -------
    `add_entry(cqa: ContextualQA)` -> None
        Adds a set of answers for a given contextual question-answer
        instance to the "generated_answers" collection. Raises an error
        if the collection is incorrect or if the input is not a
        ContextualQA instance.
    """
